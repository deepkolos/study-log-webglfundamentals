(() => {
  ...
  function restrictRange(x, min, max) {
  ...
  // utils/animate.ts
  function animate(from, to, duration, update, ease = (x) => x) {
    const state = {
      running: true,
      cb: null,
      frameId: 0,
  ...
  async function main() {
    ...
    draw();
    const animateRotateY = () => animate([0], [deg2rad(359)], 1e3, ([rotateY]) => {
      rotation[1] = rotateY;
      draw();
    }).then(animateRotateY);
    animateRotateY();
